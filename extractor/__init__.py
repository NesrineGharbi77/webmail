...
Toutes les constantes sont configurables par variables d’environnement ;
aucune dépendance à un module `config`.
"""

from __future__ import annotations

# ──────────────────── Imports de base ────────────────────
import os, io, re, tempfile, logging, unicodedata, pathlib, time, concurrent.futures, zipfile
...
TMP             = Path(os.getenv("EXTRACTOR_TMP", "/tmp/extractor_cache"))
TMP.mkdir(parents=True, exist_ok=True)

OCR_LANGS       = os.getenv("OCR_LANGS", "fra+eng")
ATTACH_LIMIT    = int(os.getenv("ATTACH_LIMIT", "20000"))   # caractères max extraits
POPPLER_PATH    = os.getenv("POPPLER_PATH", "/usr/bin")     # pour pdf2image
PDF_PAGE_MAX    = int(os.getenv("PDF_PAGE_MAX", "3"))       # pages OCR max par PDF

DESKEW_MAX_SKEW    = int(os.getenv("DESKEW_MAX_SKEW", "15"))     # ° max
DESKEW_MIN_IMPROVE = float(os.getenv("DESKEW_MIN_IMPROVE", "0.05"))
...
def _ods_snippet(data: bytes, limit: int) -> str:
    ...
    try:
        ...
        logger.warning("Erreur ODS : %s", exc); return ""
...
def _pdf_snippet(data: bytes, limit: int=ATTACH_LIMIT) -> str:
    try:
        pages=convert_from_bytes(data,dpi=200,poppler_path=POPPLER_PATH)
    except Exception as exc:
        logger.error("PDF→image KO : %s", exc); return ""
    txt=[]
    for p in pages[:PDF_PAGE_MAX]:
...
def att_snippet(
    ...
    """
    if not data:
        logger.warning("PJ « %s » vide", name); return ""

    ext = Path(name).suffix.lower()

    # 1) ZIP ignorés
    if ext == ".zip":
        logger.info("PJ ZIP ignorée: %s", name); return ""
    ...
    if raw is None:
        mime = magic.from_buffer(data, mime=True)
        logger.info("PJ « %s » : type non géré (%s / %s)", name, ext or "aucune", mime)
        return ""
    ...
    def _extract(part, fmt):
        try:
            ...
            if isinstance(cont,(bytes,bytearray)):
                enc = chardet.detect(cont).get("encoding") or "utf-8"
                cont=cont.decode(enc, errors="ignore")
